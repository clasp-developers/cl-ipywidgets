...
#+clasp(deftype binary-types () '(or clasp-ffi:foreign-data (simple-array ext:byte8 *)))
#+sbcl(deftype binary-types () nil)
...
(defmethod do-call ((self callback-dispatcher) &rest args)
  (let (value)
    (loop for cb in (callbacks self)
       do (cl-ipykernel:with-error-handling "In do-call"
...
(defun slot-default-value (widget-class slot-name)
  (let* ((class-slots (closer-mop:class-slots widget-class))
	 (slot (find slot-name class-slots :key #'closer-mop:slot-definition-name)))
    (if slot
...
(defun do-register (widget-class-name)
  (let ((widget-class (find-class widget-class-name)))
    (closer-mop:finalize-inheritance widget-class)
...
(defun call-widget-constructed (w)
  (cl-jupyter:logg 2 "call-widget-constructed widget -> ~a~%" w)
...
(defun widget-open (self)
  (multiple-value-bind (state buffer-paths buffers)
    ...
    (cl-jupyter:logg 2 "In widget-open~%")
    (cl-jupyter:logg 2 "state -> ~a~%"
		(with-output-to-string (sout)
		  (cl-ipykernel:print-as-python state sout :indent 4)))
    (cl-jupyter:logg 2 "buffer-paths -> ~s~%" buffer-paths)
    ...
    (when (> (length fstate) 0)
      (multiple-value-bind (state buffer-paths buffers)
        ...
        (let ((msg (list (cons "method" "update")
          ...
          (cl-jupyter:logg 2 "widget.send-state~%")
...
(defun slot-name-from-json-name (json-name widget-class)
  (let ((slots (closer-mop:class-slots widget-class)))
    (loop for slot in slots
       for slot-definition-name = (closer-mop:slot-definition-name slot)
...
(defmethod set-state ((widget widget) sync-data)
  (cl-jupyter:logg 2 "set-state  sync-data -> ~s~%" sync-data)
  (unwind-protect
...
(defun get-state (object &key key)
  ...
  (let ((keys (cond
    ...
    (loop for slot-name in keys
       for slot-def = (or (find slot-name (closer-mop:class-slots (class-of object))
       ...
       collect (cons (or (traitlets:traitlet-metadata (class-of object) slot-name :json-name)
			 (string (closer-mop:slot-definition-name slot-def)))
		     (funcall to-json (widget-slot-value object slot-name) object)))))


(defmethod %should-send-property ((widget widget) key value)
  (check-type key symbol)
  (cl-jupyter:logg 2 "%should-send-property key -> ~s (property-lock widget) -> ~s~%" key (property-lock widget))
              ...
              (when key-in-property-lock
                (multiple-value-bind (split-value-state split-value-buffer-paths split-value-buffers)
                  ...
                  (multiple-value-bind (split-lock-state split-lock-buffer-paths split-lock-buffers)
                    ...
                    (cl-jupyter:logg 2 "testing (and (equal split-value-state split-lock-state)
                               (equal split-value-buffer-paths split-lock-buffer-paths)
                               (%buffer-list-equal split-value-buffers split-lock-buffers))")
                    (cl-jupyter:logg 2 "split-value-state -> ~s~%" split-value-state)
                    (cl-jupyter:logg 2 "split-lock-state -> ~s~%" split-lock-state)
                    (cl-jupyter:logg 2 "split-value-buffer-paths -> ~s~%" split-value-buffer-paths)
                    (cl-jupyter:logg 2 "split-lock-buffer-paths -> ~s~%" split-lock-buffer-paths)
                    (cl-jupyter:logg 2 "(%buffer-list-equal split-value-buffers split-lock-buffers) -> ~s~%" (%buffer-list-equal split-value-buffers split-lock-buffers))
...
(defmethod %handle-displayed ((self widget))
  (cl-jupyter:logg 2 "In %handle-displayed (display-callbacks self) -> ~a~%" (display-callbacks self))
